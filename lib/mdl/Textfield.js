...
class FloatingLabel extends Component {
  ...
  _onLabelLayout = ({ nativeEvent: { layout } }) => {
    ...
    if (width && !this.offsetX) {
      this.offsetX = (( Dimensions.get('window').width - (width ) ) / 2 );
    }
    if( this.selfTextUpdate ){
      this.offsetX = (( Dimensions.get('window').width - (width ) ) / 2 );
      this.refs.label.setNativeProps({style:{ right:this.offsetX }})
      this.selfTextUpdate = false;
  ...
  updateTextAndLocation(text) {
    this.selfTextUpdate = true;
    this.setState({ text });

  }
  measure(cb) {
    if (typeof this.refs.label.refs.node == 'undefined')
      return false
...
class Textfield extends Component {
  ...
  changeFloatLabel(newText){
    this._originPlaceholder = newText;
    this.refs.floatingLabel.updateTextAndLocation(this._originPlaceholder);
  }
  componentWillMount() {
  ...
  render() {
    ...
    return (
      <Animatable.View style={this.props.style} ref='view'>
        <TextInput  // the input
          ref="input"
          autoCorrect={false}
          {...inputProps}
